...
var inputFileName = "ErnestInputScript.txt";
// order of precedence for all characters in scene - 1st is highest priority
var charprecedence = ["JACK", "LADYBRACKNELL", "MISSPRISM", "GWENDOLEN", "CHASUBLE", "ALGERNON", "CECILY"];

// all marks, pawns, miscellaneous objects with their translated standardized value
var locationarray = ["CENTER", "BACK", "UPSTAGE", "DOWNSTAGE", "FORWARD", "BACKWARD", "STAGELEFT", "STAGERIGHT", "BACKSTAGERIGHT", "RIGHT", "CENTERBACKSTAGE", "AUDIENCE", "LEFT", "CENTERRIGHT", "OUT", "AWAY", "ARMS", "FRONT",  "AHEAD", "FORWARD",  "ROOM", "CENTERSTAGE", "BOOK", "CHAIR", "TABLE", "COUCH", "HANDBAG", "BOOKCASE"];
var translatedlocationarray = ["CENTER", "/BACK", "UPSTAGE", "DOWNSTAGE", "/FORWARD", "/BACK", "STAGELEFT", "STAGERIGHT", "BACKSTAGERIGHT", "/RIGHT", "CENTERBACKSTAGE", "AUDIENCE", "/LEFT", "CENTERRIGHT", "STAGERIGHT", "STAGERIGHT", "/CURRENT", "AUDIENCE", "AUDIENCE", "AUDIENCE", "/OPPOSITE", "CENTER", "BOOK", "CHAIR", "TABLE", "COUCH", "HANDBAG", "BOOKCASE"];

// all characters and pronouns with their translated standardized value
var chararray = [ "HE", "SHE", "HER", "WHO", "HIS", "JACK", "LADYBRACKNELL", "MISSPRISM", "GWENDOLEN", "CHASUBLE", "ALGERNON", "CECILY"];
var translatedchararray = [ "/CURRENT", "/CURRENT", "/CURRENT", "/CURRENT", "/CURRENT", "JACK", "LADYBRACKNELL", "MISSPRISM", "GWENDOLEN", "CHASUBLE", "ALGERNON", "CECILY"];

// all physical locations on stage that person would need to move to, not relative position
var markarray = ["CHAIR", "TABLE", "COUCH", "HANDBAG"]; //NONE FOR THIS PLAY

// all movement words related to movement mentioned
var lookarray = ["LOOK", "TURN", "FACE", "GAZE", "LOOKS", "TO", "STARING", "STARES", "GLANCE", "GLANCES", "BENDS", "PEERS", "SIGHT", "CATCHES"];
var walkarray = ["WALK", "GO", "MOVE", "ENTER", "CROSS", "CROSSES", "STEP", "STEPS", "EXITS", "GOES", "BESIDE", "TO", "SITS", "ENTERS", "EXIT", "RETURNS", "COMES", "MOVES", "COME", "BRINGS", "EMBRACES", "EMBRACING", "SIEZES", "SINKS", "SHAKES", "RUSHING", "RUSHES", "APPROACHES"];
var pointarray = ["POINT", "GESTURE", "TOUCHES", "HUGS", "WIPES", "POKES", "POKING", "EMBRACE"];
var pickuparray = ["PICK", "PICKUP", "LIFT", "PICK-UP", "CARRY", "CARRYING", "CARRIES", "HOLDS", "PICKS", "GIVES", "TAKES", "TEARS"];// removed with & moves
...
// CJT 7/7/16					return gravedigger1.name;
	...
	function isPropLine(data) {
		...
		if(pos != -1 && temp[0] != '/') {
			...
			console.log("ispropline got translated value" + temp);
			return temp;
		}
	...
	function calcPosition(data, actor) {
		...
		switch (data) {
			case "/BACK":
				...
				var target = {name: "/BACK", x: result[0], y:result[1]};
				return target;
				break;
			case "/FORWARD":
				...
				result[0] = x;
				result[1] = y + 550;
				var target = {name: "/FORWARD", x: result[0], y:result[1]};
				return target;
				break;
			case "/RIGHT":
				...
				var target = {name: "/RIGHT", x: result[0], y:result[1]};
				return target;
				break;
			case "/LEFT":
				...
				result[0] = x + ((50) * Math.sin(actor.angle + (Math.PI / 2)));
				result[1] = y + ((50) * Math.cos(actor.angle + (Math.PI / 2)));
				var target = {name: "/LEFT", x: result[0], y:result[1]};
				return target;
				break;
// ****************************************************************************************
...
// ****************************************************************************************
			...
			case "/OPPOSITE":
				...
				result[0] = 530 - diff;
				result[1] = actor.y;
				var target = {name: "/OPPOSITE", x: result[0], y: result[1]};
				return target;
				break;
			case "/COIN":
...
console.log("translate target2="+((target == null)?("null"):(((target instanceof Character || target instanceof Pawn)?(target.name):(target)))));
		...
		if(!( target instanceof Array) && target != null && target.name[0] != "/") {
			var temp = findItem('C', target.name);
			if(temp == null) {
				temp = findItem('O', target.name);
			}
			if(temp != null) {
				target = temp;
			}
		...
		if(firstverb == -1) {
			...
			if(allnouns[0] != -1) {
				...
				if(secondnindex == -1) {
					...
					console.log("first="+firstnoun+":"+firsttype+", second="+secondnoun+":"+secondtype);
					if (firstnoun[0] == "/")  {
					...
					if(actor == null) {
						...
						if(secondnoun[0] == '/') {
							...
							console.log("second noun calc postn gives target="+target.name);
							if (findItem('C', target.name) != null) {
								// do nothing
							} else {
...
// **************************************************************************************
	...
	var backstageright = new Pawn('BACKSTAGERIGHT', 2500, 209, '255,255,255', '255,255,255', false);
	
	var book = new Pawn('BOOK', 105.59, 68.60, '0, 0, 0', '234, 234, 234', true);
	
	var chair = new Pawn('CHAIR', 1522.25, 857.17, '255,255,255', '255,255,255', false);
	var table = new Pawn('TABLE', 861.14, 725.74, '255,255,255', '255,255,255', false);
	var couch = new Pawn('COUCH', 955.59, 594.32, '255,255,255', '255,255,255', false);
	var handbag = new Pawn('HANDBAG', 1994.47, 68.60, '255,255,255', '255,255,255', false);
	var bookcase = new Pawn('BOOKCASE', 105.59, 68.60, '255,255,255', '255,255,255', false);


	// create characters

	var jack = new Character('JACK',1238.92, 857.17, 0, '135,206,250', '0,0,128');
	var ladybracknell = new Character('LADYBRACKNELL', 1050.03, 1514.32, 0, '171,130,255', '85,26,139');
	var missprism = new Character('MISSPRISM', 1994.47, 68.60, 0, '0,205,0', '0,100,0');
	var gwendolen = new Character('GWENDOLEN', 1333.36, 988.60, 0, '233,150,122', '139,69,0');
	var chasuble = new Character('CHASUBLE', 1654.47, 1172.60, 0, '233,150,122', '139,69,0');
	var algernon = new Character('ALGERNON', 710.03, 594.32, 0, '233,150,122', '139,69,0');
	var cecily = new Character('CECILY', 766.70, 541.74, 0, '233,150,122', '139,69,0');


	// who to default context to if no context person defined yet
	var defaultchar = jack;
	var defaulttarget = backstageright;
	
	// all physical objects and people to be represented in the UI
	var mynames = ["JACK", "LADYBRACKNELL", "MISSPRISM", "GWENDOLEN", "CHASUBLE", "ALGERNON", "CECILY", "BOOK"];
